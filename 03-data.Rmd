...
"CSV" stands for **c**omma **s**eparated **v**alue. In other words, csv files these are files whose values are separated by commas. Each comma from the csv file corresponds to a column, and the column names are taken from the first line of the file. The function then "guesses" an appropriate data type for each of the columns.

* `write_csv()` saves data as a csv file in R. This function has two main arguments: `x` and `path`. The `x` argument is the data set that you want to save. The `path` argument is the file path where you want to save the file. The end of the path argument is the name that you want to use for the file. 

* `read_csv()`, which is from the **readr** package, reads a file back into R. The read_csv has one main argument: `file`, which is the file path to the file that you are wanting to read into R.

For example, the link below is for a file containing faculty gender data across departments at Harvard University. Note, the `file` argument of the `read_csv()` function is very flexible. Below, it takes a url. `file` can also be the absolute or relative path for a file saved locally on your computer. 
...
We need to "skip" the first two rows of the file because there are code comments at the top, which has nothing to do with the actual data.
...
Do you see the column specification message? As of now, R "guesses" an appropriate data type for each of the columns. To get rid of this message, we tell R the column data types.
...
`read_csv()` reads files whose values are separated by commas, i.e. delimited by commas. However, in the real world, some data may use different delimiters other than commas such as pipes. In this case, we can use `read_delim()` to read the file. The function `read_delim()` takes the arguments as `read_csv()`. The only difference is the `delim` argument needs to be specified. 
...
In order to write excel files you have to install complex packages, and they are hard to create. Writing excel files is beyond the scope of Primer. However, reading excel files is easy. To do so, we use the `read_excel()` function from the **readxl** package.


```{r}
library(readxl)

file_2 <- "https://raw.githubusercontent.com/PPBDS/primer.tutorials/master/inst/www/excel_1.xlsx"

read_excel(file = "excel_1.xlsx" )
```
...
<!-- BG: pick example of json file (could be from library) stick it in data directory w/ faculty.csv, submit PR just for this -->

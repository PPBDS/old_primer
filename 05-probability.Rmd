...
<!-- Add copy button to tutorial in correct places. -->

<!-- Read https://ppbds.github.io/primer.tutorials/articles/instructions.html -->
...
*Mathematical distribution* is complete based on mathematical formula and assumptions like in the Flipping a coin session we assume that the coin is an perfectly fair coin where where the probability landing on heads or tails is equal. In this case, we assume that the average hight of men is 178 cm, as well as the standard deviation for height is around 7.5 cm. When we have these two values, the average which we also called the mean, and standard dieviation (sd), we can create an normal distribution using the `rnorm()` function. And an normal distribution is an good approximation and generalization for height in our sinario.


Mathematical Distribution:

```{r}
tibble(height = rnorm(10000, mean = 178, sd = 7.5)) %>% 
  ggplot(aes(x = height)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                 binwidth = 1, 
                 color = "white")+

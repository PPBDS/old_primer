...
<<<<<<< HEAD
<!-- Make a bunch of comments with your initials. Done.-->

<!-- Skim chapter 4. Read chapters 6 and 7. Done.-->

<!-- Understand the code. Learn a bit about https://www.tylermw.com/3d-ggplots-with-rayshader/. Done.-->


<!-- Old comments to discuss: -->

<!-- Start thinking about how to expand the tutorials. What should they cover so that students start to think about probability and distributions easily and naturally? Check out the chapter 4 tutorials. -->
=======
<!-- Make this warning go away: Warning in make_shadow(heightmap, shadowdepth, shadowwidth, background, : -->
<!-- ## `magick` package required for smooth shadow--using basic shadow instead. -->

<!-- Start thinking about how to expand the tutorials. What should they cover so that students start to think about probability and distributions easily and naturally? Do the chapter 4 tutorials. Including the written answers.  -->
>>>>>>> 3990a569075371f60dd283aaa6bc4954f3c072db

<!-- Introduce the concepts of population and, maybe, exchangeability. Need to set the stage for making an assumption about one population in chapter 6. -->

<!-- Explain map_* functions. Maybe we don't need list-columns? Or should we keep them? Or maybe we should change the code around so that they are not necessary? -->

<!-- Learn about movies in rayshader. -->

<!-- Consider the parallel structure of the 2-model, 3-model, and n-model case. First, we should always begin by discussing how to create the joint distribution of possible models and possible experimental results. This is done by, first, assuming that one of the models is true and then, second, show the variation in the world which results. That is, even if we know for a fact that you do not have the disease, there is still a chance that you test will come back positive. So, we should do that for one model. Then, we create the joint distribution by doing that same thing for all possible models. (We show all this code.) The joint distribution is a tibble which we are happy to look at. It always has two columns: the first is the possible models and the second is one possible result (which will vary) if that model is true. So, each possible model will have 1,000 (or at least 100) rows in the joint distribution. Do you agree that we have done this? If so, rewrite this comment and place it as a comment at the start of two models so that future authors know what we are doing. And the next two comments as well. -->
...
<!-- After rayshader, we then use the joint distribution to make two graphics, showing the posterior distribution for the model. The first is unnormalized and the second is normalized. In both cases, we begin with the joint distribution --- an object we have saved --- and then use filter to get our slice. Maybe all this can be done in a rayshader movie as well. -->

<!-- Enough for today. -->

<!-- Should each section also do something with the resulting posterior? Use it to answer a question. Yes! Again, the N Model example is currently the best example. Note how this connects to the Temperance examples in each chapter. Maybe make this explicit? -->


<!-- Maybe add, after N models, a section on Infinity models, meaning a continuous parameter p. Note that we have a continuous distribution section now. Want to set the stage for Chapter 6, in which we are generating a posterior for p which we may think of as taking an infite number of values. This is a key dimension on which chapter 5 and 6 need to connect. -->
...
<!-- Need more discussion about quintiles and area under the curve. Maybe simple examples of this are done for each of N = 2, N = 3 and N = N models.  -->
...
<!-- YW : I think this is my first time learning about ggplot information in lists (maybe I have seen an error because of that) So maybe adding parts in Visualization unit that foreshadow this part. -->
...
<<<<<<< HEAD
<!-- YW:Not sure what is "can be either discrete or continuous" means in this content? Is it like scale_x_discrete, or something else? -->
=======
<!-- YW:Not sure what is "can be either discrete or continuous" means in this content? Is it like scale_x_discrete, or something else? DK: So fix it! -->
>>>>>>> 3990a569075371f60dd283aaa6bc4954f3c072db
...
<!-- YW: Good on elaborating and expanding on distribution and probability distribution as well as bringing in "Unnormalized" distribution in the content and how it connects to the previous two.  -->
...
<!-- YW:Maybe a better explanation but I would say good for now. -->
...
<<<<<<< HEAD
<!-- YW: Elaborate more on "The probability distribution is the Platonic Form" I found it hard to understand, I would say either explain more or simply deleted.  -->
...
<!-- YW: "The proportion of heads to tails should not be too difference if this coin is fair" I get the point, I think it would be better if connects to the large chunk of explanation of $\rho$ and specific probability in this discussion. -->
=======
<!-- YW: Elaborate more on "The probability distribution is the Platonic Form" I found it hard to understand, I would say either explain more or simply deleted. Agreed! Elaborate! -->
...
<!-- YW:I like this explanation. -->
<!-- YW: Beliefs for certain hypothesis after the evidence -->
...
<<<<<<< HEAD
<!-- YW:Make sense to me a person learned math, might need pre-existing math/number common sense -->
<!-- YW:mathematical distribution - data based on mathematical models/most ideal situation. -->
=======
>>>>>>> 3990a569075371f60dd283aaa6bc4954f3c072db
...
<!-- YW: Empirical distribution-data based on trial/simulation -->
...
<!-- YW: Posterior distribution based on beliefs and existing hypothethis.  -->
...
<!-- YW:Ev means number of electoral college vote for states for example rep(3,8) means there are 8 states in the US that have the electoral college vote of 3 votes. -->
...
<!-- DK: Change this example to heights, something that is truly continuous. -->
...
```{r, echo = FALSE}
tibble(p = rbeta(1000000, 200, 180)) %>% 
  ggplot(aes(x = p)) +
    geom_histogram(aes(y = after_stat(count/sum(count))), 
                   bins = 500) +
    labs(title = "Posterior Probability Distribution",
         subtitle = "Percentage of Americans who approve of President Biden",
...
<!-- YW: A little overwhelmed by the above. -->
...
<!-- YW:Maybe elaborate little on what those number mean in the context of presedential election.  -->
...
<!-- YW: Normalization is used when we convert an data that is cannot be expressed by probability into something that we can expressed by probability? -->
...
<!-- YW : Joint distribution is used when we have more than one outcome for an distribution. -->
...
<!-- YW: Why we need a 3D plot is it why 2D isn't sufficient enough, abit confused here why is it not sufficient?. -->
```{r, echo = FALSE}
...
<!-- YW : So that the probability for some combination decreased as the tree branch out.  -->
...
<!-- YW: I kinda see where this is going, p(A|B) is difference from p(B|A) if I remember right that is part of the Bayes Theorem right?-->
...
<!-- YW: If I understand correctly the scenario here is 1st, "what is the probability that you actually have the disease if you test positive?" which is P(disease | Test=positive) = 95% as well as "What is the probability that you actually don't have the disease if you test negative?" which is P(No diseases| Test=negative) = 95%, vice verse, so if I understand it correctly there is four possibility.  -->
<!-- YW: P(Disease | Test=positive)=95% -->
<!-- YW: P(Disease | Test=negative)= 5% -->

<!-- YW: P(No Disease | Test=positive)= 5% -->
<!-- YW: P(No Disease | Test=negative)=95% -->
<!-- YW: Where each of the two branches added up to 100%. -->
...
<!-- YW: Why we need 3D for this one? I found that the 4 box in the plot easier to understand than the rayshader plot. -->
...
jd_marbles_plot <- jd_marbles %>%
  ...
  ggplot(aes(in_bag, in_sample, color = total)) +
    geom_point() +
...
<!-- YW: Never heard of it before this course but good to know! -->
...
After Wisdom, we have an *actual Preceptor Table.* It includes rows for the data we have and the data we want to have. It has missing values, most importantly for potential outcomes which were not observed. The central problem in inference is to fill in the question marks in the actual Preceptor Table.
...
<!-- YW:Giving hand to the next few chapter. -->
...
<!-- YW:Okay but a little vague, don't know where but feeling unclear.  -->
...
```
<!-- YW: Cool, I would say that this chapter contained a whole lot of theory and mathematics stuff, rather than direct application like the visualisation Unit. I found the some of the models pretty confusing and it sometimes take me a while to determine what each variable in p(a|b) is. Not sure the Cardinal Virtues should be put here. Needs better connection with the next and later chapters, maybe us good to introduce sampling or at least foreshadow it at somepoint.Chap.6 done a great job of including parts of chapter 5 like Joint distribution, posterior distribution, etc. Both Chap 6 and Chap 7 has refer back to the Cardinal Virtue so I guess it's not a good idea to switch the position as it is used throughout later chapters.  -->

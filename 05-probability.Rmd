...
<!-- Joint distribution in disease example and marble example are wrong. Change disease example so that we have 50% accurate if you have the diseases and 99% accurate if you don't. -->

<!-- When building the chapter, we don't want rayshader images to pop up? -->

<!-- Learn about movies in rayshader. -->

<!-- Make this warning go away: Warning in make_shadow(heightmap, shadowdepth, shadowwidth, background, : `magick` package required for smooth shadow--using basic shadow instead. .-->
...
Could includes population in two models where we simulate the results for 100,000 people of testing data for the disease (prolophobia), the 100,000 people is the size of our population which is also the N in chapter 6 . -->

<<<<<<< HEAD

<!-- Start thinking about how to expand the tutorials. What should they cover so that students start to think about probability and distributions easily and naturally? Do the chapter 4 tutorials. Including the written answers.Done.-->
...
map function and map_* functions could turn an data (vector or list) and always results in lists (list Column) there are two parts in the map() function as well as map_*() function the .x and .f, the '.x' and '.f' placed in the map functions and map_* like this map(.x,.f) where .x as could either be a list or a vector, and .f either be a direct function like .f=mean, or an formula like .f= ~mean(.x), remember to put the "~" if you are using like an formula, the difference between map and map_* function is when you know and want the outcome of the data to be specific data vector like (double, logical,character,integer) rather than an general (list) in map(), you can use map_* instead of map to organized your list_column.  Done.-->

<!-- Consider the parallel structure of the 2-model, 3-model, and n-model case. First, we should always begin by discussing how to create the joint distribution of possible models and possible experimental results. This is done by, first, assuming that one of the models is true and then, second, show the variation in the world which results. That is, even if we know for a fact that you do not have the disease, there is still a chance that you test will come back positive. Good
So, we should do that for one model. Then, we create the joint distribution by doing that same thing for all possible models. (We show all this code.) The joint distribution is a tibble which we are happy to look at. It always has two columns: the first is the possible models and the second is one possible result (which will vary) if that model is true. So, each possible model will have 1,000 (or at least 100) rows in the joint distribution. Do you agree that we have done this? If so, rewrite this comment and place it as a comment at the start of two models so that future authors know what we are doing. And the next two comments as well. Yes we have it,  rewrite and paste front of two models. Done.-->

<!-- Having create the joint distribution, we graph it in two dimensions. (We show the code for that.) Then, we graph it in three dimensions with rayshader. (Maybe there is a movie. Maybe we swoop in and fly around it.) Then, we take a "slice" of that joint distribution via rayshader. Move that slice around. Et cetera. Not sure how to do all that . . . Note that you can make rayshader objects interactive with rgl::rglwidget(). The ggral (sp?) package might also be useful (Did not use ggral yet to create rayshader 3D).Done. -->
=======
>>>>>>> bb5392527871675aab6b350a785ef7485fbc172d

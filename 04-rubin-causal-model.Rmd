...
<!-- Re-arrange and add rows to the first Population Table. First population, then data, then population, then Preceptor Table, the population. -->

<!-- Add a Covariate column for Gender to Two values for tau. Explain what a covariate is. (One sentence.) This allows us to know which units have causal effect \tau_f and \tau_m. -->

<!-- Begin the Assumptions section by making your first real Population Table: including a Source column and a Time column. Not sure if you should use the height example or the train example. But we need a full example in order to make clear what the assumptions refer to. -->

<!-- In Representativeness section, we need a table which shows all the rows which we might have gotten when we got our data. This includes two kinds of rows: the data we got and the data we did not get (but might have gotten). We can be sure that the data is "representative" if it includes all the data we could have gotten or if it is a random sample from that data. This is *not* the same thing as all the rows in the Population Table. This is a subset of the population table, all the rows with the value for the Time column. -->
...
<!-- DK: Things to fix in this table. Remove gender. Add ID. Always include the Year. Only need two rows from each source. Order by year. So, order is population (for 2010 or whatever), data, population (for 2018), preceptor table, and population (for 2025 or whatever).  -->
...
Implicit in the Preceptor Table is a notion of time. Now that we can see our actual data compared with our greater population and our desired data, we must expand our observations. That is to say that, given our data is sourced from 2015 and our desired data is from 2021, we must include a greater time span in our predictions. In working with the Population Table, there are three key assumptions to consider: validity, stability and representativeness.


### Validity 

To understand validity in regards to the Preceptor Table, we must first recognize an inherent flaw in any experiment design: *no two units receive exactly the same treatment*. 
...
Indeed, there are an infinite number of possible treatments. The Preceptor Table extends to the right forever. The validity assumption allows us to "collapse" all the different treatments into a single column called treatment. 

<!-- DK: Above is too brief and awkward. -->
...
Stability means that the relationship between the columns is the same for three categories of rows: the data, the Preceptor table, and the larger population from which both are drawn. 
 ...
 <!-- More meat here. Some examples, using height and immigration attitudes, from above. When is it OK to assume stability and when is it not? What is a reasonable time frame to use for the population?  -->

...
<!-- Use new Preceptor Table and Population Table formats in Chapters 8 through 11. -->

<!-- Changes discussions in Chapter 8 through 11 about validity, stability and representativeness. -->

<!-- Population Table, by definition, always has three columns: Source, Time and ID (in that order), although Time might be replaced with the unit of time, like Year or Date. Think about just placing the Preceptor Table (and the data set) within a larger context. MF: Done. -->

<!-- All Population Tables have lots of ... (to indicate missing rows) since we need to highlight that the Population is big. MF: Done. -->

<!-- It is typically the case that the order of the Source's in the Population Table is Population, Data, Population, Preceptor Table, Population. That is, the Data and the Preceptor Tables are almost always assumed to be have been drawn from a population which covers the time periods before, between and after the data and the Preceptor Table. MF: Done. -->

<!-- Row for Population Table under assumptions are wrong. We have the outcome under either treatment or control for our Data rows, not for Population or Preceptor rows. MF: Done. -->
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Tahmid", "Diego"),
  ...
  gt() %>%
    ...
    cols_align(align = "center", columns = everything()) %>%
    cols_align(align = "left", columns = c(subject)) %>%
    tab_spanner(label = "$$\\text{Outcomes}$$", 

...
<!-- Re-arrange and add rows to the first Population Table. First population, then data, then population, then Preceptor Table, the population. MF: Done. -->

<!-- Add a Covariate column for Gender to Two values for tau. Explain what a covariate is. (One sentence.) This allows us to know which units have causal effect \tau_f and \tau_m. MF: Done.  -->

<!-- Begin the Assumptions section by making your first real Population Table: including a Source column and a Time column. Not sure if you should use the height example or the train example. But we need a full example in order to make clear what the assumptions refer to. MF: Done. -->
...
```{r, echo = FALSE}
tibble(source = c("Population", "Data", "Data", "Population", 
                  "Preceptor Table", "Population"),
       year = c("2010", "2015", "2015", "2018", "2021", "2025"),
       age = c("?", "18", "23", "?", "19", "?"),
       height = c("?", "180", "163", "?", "?", "?")) %>% 
...
tibble(subject = c("Yao", "Emma", "Cassidy", "Tahmid", "Diego"),
  ...
  tab_spanner(label = "$$\\text{Outcomes}$$", c(ytreat, ycontrol)) %>%
  tab_spanner(label = "$$\\text{Causal Effect}$$", c(ydiff)) %>%
  tab_spanner(label = "$$\\text{Covariate}$$", c(gender)) %>%
  fmt_markdown(columns = everything())
```
...
tibble(source = c("Population", "Population", "Data", 
                  "Data", "Preceptor Table", "Preceptor Table", 
                  "Population", "Population"), 
       year = c("2012", "2012", "2014", "2014", 
                "2021", "2021", "2023", "2023"),
       sex = c("?", "?", "Male", "Female", 
               "Female", "Female", "?", "?"),
       ytreat = c("13", "?", "?", "?", "?", "?", "?", "?"),

...
<!-- Take a look at the new version of Cardinal Virtues in Chapter 5. It is not perfect! But it is not. -->

<!-- Reserve the term "population" for the statistical definition: the imaginary urn from which we draw data, or could have drawn data. -->

<!-- MF: Focusing on organization now. After removing the "Complications" section, relevant topics were added to other parts of the chapter. Main issue is finding where to include our new validity/stability/representativeness concepts. Perhaps this discussion should replace the newly created "validity" section? Or added separately? -->

<!-- Maybe (!) move more of the Complications (like Data Problems?) earlier in the chapter. Like Data Problems under Preceptor Tables? MF: Done. -->

<!-- Populations again section is especially a mess! Idea is to talk more about populations now that we have more of a framework in which to consider things. Make the distinction between the Preceptor table and the larger population from which it is drawn. MF: Done. -->

<!-- Note that Internal/external validity section used to be after Data Problems in Complications. Now it is ahead! Unless we move Data Problems to Preceptor Table section . MF: Fixed. -->

<!-- The Population Table always has a column for Time, by definition. Even in the case, like Chapter 6, with a single urn, we have the urn at 11:00 (when we drew our sample) and the urn at 11:05, when we will draw our second sample. The Population Table has that urn, at 5 minute intervals, from 9:00 to 5:00, or whatever. And so, every row in the Population Table is specified by an ID and Time combination.  MF: How do we show this with Height example? Data = Yale in 2018, Preceptor Table = Harvard in 2021, Population = Ivy League students from 2015-2020?  -->
...
We want to make Representativeness have the same nice/clean connection to the infinite Preceptor Table which Validity has. Wouldn't that be nice! Can't do it! In fact, Validity is also connected to the rows. Or maybe it is Stability. 
...
<!-- Switch all this to Harvard students example? MF: Done. -->

...
<!-- MT: we should split between one causal and one predictive question. do predictive question first regarding the expectation and then causal question next regarding whether winning the election affects their longevity -->
...
tibble(ID = c("Candidate 1", "Candidate 2"),
  ...
  tab_spanner(label = "Outcome", columns = c(yeffect)) |>
  tab_spanner(label = "Covariate", columns = c(sex, before, ytreat, ycontrol))
```
...
### Data Generating Mechnaism

<!-- MT: need to include the mathematics here -->
<!-- MT: if there are separate mathematics for each section would it make sense to do something similar to three parameters? break up each section based on sex, election_age, election_age and sex, and then election_age, election_age, sex and election_age*sex. each section would then include its own data regarding the cardinal virtues. each could potentially answer each of the four different questions above?-->
...
*More broadly, how long do candidates, in general, live after the election?*
*Does winning the election affect their longevity?*

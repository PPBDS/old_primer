...
In our graph above, we use the function `scale_x_continuous()` because our x-axis variable is continuous, it can take on two particular real values. `scale_x_continuous()` converts our x-axis to having two different "tick marks". **Note:** There is a fairly even distribution of Tails and Heads. More draws would typically result in an even more equal split. 

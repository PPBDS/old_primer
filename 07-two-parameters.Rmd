...
<<<<<<< HEAD
<!-- Read chapter 4 through 6. GP: DONE -->

<!-- Re-read 5.7 and note ways that chapter 7 is not consistent with it.
GP: There are two big things. First, I think the introduction of 7.1 (everything before 7.1.1) belongs outside of the "Wisdom" section entirely. Chapter 5 establishes that the cardinal virtues are parts of a process to answer your questions. Creating the questions themselves is not part of "Wisdom" (as per chapter 5) and therefore doesn't belong in that section, but rather right before it. Secondly, it seems to me that most of what is discussed in the "Courage" section actually belongs in the "Temperance" section.  -->

<!-- Read chapter 8. GP: A bit. -->

<!-- Add lots of comments to this chapter, using your initials. GP: Added enough comments to keep me busy for now. I know what I'll need to work on now. -->

<!-- Do the tutorial. GP: DONE -->

<!-- Read my comments. Make sense? GP: DONE -->

<!-- Check MF's claims about being done for truth. GP: DONE (Her claims are true) -->
=======
<!-- There are two big things. First, I think the introduction of 7.1 (everything before 7.1.1) belongs outside of the "Wisdom" section entirely. Chapter 5 establishes that the cardinal virtues are parts of a process to answer your questions. Creating the questions themselves is not part of "Wisdom" (as per chapter 5) and therefore doesn't belong in that section, but rather right before it. Secondly, it seems to me that most of what is discussed in the "Courage" section actually belongs in the "Temperance" section. Agreed! Do this.  -->

<!-- Delete females from EDA plot and make a better plot.  Maybe it has both a histogram and a density overlayed. Or maybe not! I don't know. -->
>>>>>>> 28c2ed2de15f0a1278ef1ffbc158e9533c4d47db

<!-- old stuff: -->

<!-- Reserve discussion of validity for cases in which the measure/meaning/treatment has truly changed. Examples? Meaning if height has not truly changed. Or has it? Are we measuring people with shoes on or shoes off? Height is not just height. It is all the details associated with how you took that measurement. Where did the data come from?  Are you telling people to stand up straight? What tool are you using to measure? MF: Done. -->

<!-- Remove ideal from ideal Preceptor Table. After chapter 4, there is only one notion of a "Preceptor Table." It is a table with the rows and columns such that, if there is no missing data, you can answer all the questions with easy algebra. MF: Done. -->
...
<!-- This EDA is not very good and does not build naturally to where we want to be. -->
...
Interesting! There are 353 missing values of height in our subset of data. Just using `glimpse()` does not show us that.  Let's filter out the NA's using `drop_na()`. This will delete the rows in which the value of any variable is missing. For simplicity, and because we want to examine height in men (not boys), let's limit our data to only include adults. 
...
We see the most probable heights for both genders and that men are generally taller than women. 

Let's focus on a subset of the `nhanes` data, designed to answer our questions. And, instead of using all the data, let's just use 50 randomly selected observations. (The `set.seed()` function ensures that the same 50 observations are selected every time this code is run.) 
...
Even though the original question is about “males” in general, and does not specifically refer to those males in the United States, we will assume that the data we have for United States citizens is, uh, representative enough of the population we are interested in. If we did not believe that, then we should stop right now. The major part of Wisdom is deciding what questions you can’t answer because of the data you just don’t have.
...
<<<<<<< HEAD
<!-- GP: I feel as though the entire 7.1.3 section can be summarized in one paragraph and placed as an introduction to the subsequent section — I don't think the message merits its own entire section. -->
=======
<!-- I feel as though the entire 7.1.3 section can be summarized in one paragraph and placed as an introduction to the subsequent section — I don't think the message merits its own entire section. -->
>>>>>>> 28c2ed2de15f0a1278ef1ffbc158e9533c4d47db

### The Population Table

The Population Table shows the data that we actually have in our desired population. It shows rows from three sources: the Preceptor Table, the data, and the population outside of the data (rows which exist but for which we have no data).

In our Preceptor rows, we have information for our covariates of sex and year. However, because those rows are not included in our data, we do not have any outcome results (height). Since our scenario pertains to a prediction for average height in 2021, the year column will read "2021" and the sex column will read "male."

The rows from our data have everything: the covariates and the outcomes. The covariates here will be 2009-2011 for year and male for sex, since these are the pieces of information that are included in our data. We *will* have outcome results in these rows. 
...
As we've stated above, representativeness involves our rows. More specifically, whether the rows that we *do* have data for are the same as the rows for which we *do not* have data. This is a somewhat convoluted way of saying: do all pieces of desired data have an equal chance of being included in our dataset?

<<<<<<< HEAD
<!-- GP: The equivalence of the two above statements is not immediately apparent. -->
=======
<!-- The equivalence of the two above statements is not immediately apparent. So fix it. -->
>>>>>>> 28c2ed2de15f0a1278ef1ffbc158e9533c4d47db

Ideally, data is a random selection from our population. For our `nhanes` data, is this the case? It is time to investigate. 
...
<<<<<<< HEAD
<!-- GP: I think \epsilon_i \sim N(0, \sigma^2) warrants elaboration, as drawing an error term from a normal distribution like this is not something that has been discussed previously. -->
=======
<!-- \epsilon_i \sim N(0, \sigma^2) warrants elaboration, as drawing an error term from a normal distribution like this is not something that has been discussed previously. -->
>>>>>>> 28c2ed2de15f0a1278ef1ffbc158e9533c4d47db
...
<<<<<<< HEAD
<!-- GP: This may be of greater use earlier in the section. -->
=======
...
<<<<<<< HEAD
<!-- GP: There is an issue with how the subsections here are formatted. -->

<!-- GP: 7.3 is by FAR the most confusing section. Code here needs to be better introduced (such as in the earlier chapters) and unfamiliar terms such as "predictors" need to be more clearly defined. I believe the majority of my work on the chapter will be in fixing this section. -->
=======

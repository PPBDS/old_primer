...
<!-- RS: Add foreshadowing to Ch. 7, saying something like: "To try and estimate the probability distribution of a single parameter, p." Ch.7 does a good job of explaining 1 param v. 2 params, but readers should know what they are getting into. -->

Use the **tidyverse** package. 
>>>>>>> ed41a3b26776b175b572983de268435d0e4bc0e4
...
<<<<<<< HEAD
Recall that $p$ is the true value of the proportion of red beads. There is only one $p$. Our guesses at the proportion of red beads are known as $\hat{p}$ (pronounced p hat), where $\hat{p}$ is the estimated value of $p$ which comes from taking a sample. There can be an infinite number of $\hat{p}$. 
=======
Recall that $p$ is the true value of the proportion of red beads. There is only one $p$. Our guesses at the proportion of red beads are known as $\hat{p}$, where $\hat{p}$ is the estimated value of $p$ which comes from taking a sample. There are many possible $\hat{p}$'s. You and I will often differ in our estimates. We each have a different $\hat{p}$ even though we agree that there is only one $p$. 
>>>>>>> ed41a3b26776b175b572983de268435d0e4bc0e4
...
What we just demonstrated in this activity is the statistical concept of **sampling**. We want to know the proportion of the urn's beads that are red. Performing an exhaustive count of the red and white beads would be time-consuming. Therefore, it is much more practical to extract a *sample* of 50 beads using the shovel. Using this sample of 50 beads, we estimated the proportion of the urn's beads that are red to be 34%.
...
=======
<!-- RS: We could add a paragraph describing how the IRL sample creates an empirical distribution, while the virtual sampling creates a joint distribution model to later base our posterior distribution off of. Chapter 5 has limited real world applications, so I think it is definetly worth it to sprinkle some throughout this chapter where we can.  -->
>>>>>>> ed41a3b26776b175b572983de268435d0e4bc0e4
...
Our virtual urn needs a virtual shovel. We use `slice_sample()` and list-column mapping wizardry learned in Section \@ref(list-columns-and-map-functions) to take a sample of 50 beads from our virtual urn. This is where the `ID` column in our `urn` becomes relevant, because `slice_sample()` will sample rows with a unique ID. Without the `ID` column, the urn would be a mess of the words "red" and "white," and `slice_sample()` would have no way of telling whether or not a bead has been drawn before.  
...
<!-- RS: Also, why do we use slice_sample()? I feel like normal sample() is better, and we could get rid of having to assign IDs to each bead which serves no real purpose. -->
...
Before plowing ahead, it's important to stop 
...
Lets look back on the concept of **sampling variation** that we discussed previously. Because the number of beads drawn will vary from sample to sample, it is not good enough to estimate the proportion of beads in the whole urn from one measly sample. We need to use the virtual shovel multiple times! 
...
#Create a tibble with a sequence from 0 to urn_size. In an urn of size 1000, 0 signifies there are no red beads in the urn, while urn_size signifies all the beads are red. 

# n represents the population, and will be a list of length 1000 consisting of the words "red" and "white." The number of red beads is equal to the ID, and the number of white beads is equal to urn_size - ID.

# p represents the population proportion of red beads, and is found by dividing the total number of red beads in the population by the population size. 

# Take a sample of each population with a sample size indicated by shovel_size, and find the number of red beads in the sample. Repeat this for the given number of reps in order to reduce sample variation. 

# red_in_shovel will store the results from each experiment in a list, so unnest to remove the list and make the tibble longer. 

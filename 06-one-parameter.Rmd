...
To better visualize this distribution, take a look at the 3D version of this plot. Note the saddle shape of the graphic. By this, we mean to note the peaks at the areas where the number of red beads in the shovel is close to 0 and close to 50. Furthermore, as the number of red beads approaches halfway (25 red beads sampled from the urn), the "saddle" becomes the shortest. This is where the 3D model becomes especially handy, as it helps us visualize *variance*. When $p$ is close to 25, our "saddle" is wide because there are many possible population proportions that could yield a mid-sized number of red beads.  However, when we draw an extreme number of beads from the urn (extreme meaning close to 0 or 50), it is very likely the population proportion is close to 0 or 1, so there are fewer values for $p$ that are likely, and our "saddle" becomes narrower. 
...
joint_dist_p <- joint_dist %>%
  ...
  filter(red_in_shovel > 0 & red_in_shovel < 50) %>%
  ggplot() +
...
<!-- RS: Remove references to rayshader slicing for now, add back if there is time later.  -->
Let's examine our posterior distribution. First, the x-axis only extends to a $p$ value of .6 (rather than 1) due to the fact that, when the shovel only draws 17/50 = .34 = 34% red beads, it is almost impossible for the proportion of red beads in the urn to exceed .6. Using our distribution we can see that when we draw 17 beads it is most likely that the *actual* proportion of red beads in the urn, is somewhere in between .29 and .35. This is signified by the peak between these points. The probability is lower below .28 and above .35 because it is less likely that this is the number of red beads in the urn, conditional on the fact that the shovel drew 17/50 red beads. This is our posterior distribution. 

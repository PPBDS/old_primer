...
=======
# nothing to do with the ID variable in the urn we made previously. The former
# refers to each sample we take from the urn. The latter refers to each
# individual bead in the urn.
>>>>>>> 4deb0ad0009b4894a5c0d7c68f26562efb78da79
...
Let's look back on the concept of **sampling variation** that we discussed previously. Because the number of beads drawn will vary from sample to sample, it is not good enough to estimate the proportion of beads in the whole urn from one measly sample. We need to use the virtual shovel multiple times to obtain a more precise estimate of the proportion of red beads in the whole population.
...
3. Use the virtual shovel 1,000 times to examine the effect of more samples on the sampling distribution.   
...
# After we create a tibble with 3 rows, one for each number of trials, we create 
# a range from 1 to . , where . passes in the number or trials using mapping.
  
# For example, when reps = 10 we create a range from 1 to 10, and when reps = 33
# we create a range from 1 to 33. 
  
  
  mutate(trial_ID = map(reps, ~1:.)) %>%

# unnest() is a nifty function from the tidyr package, which expands a tibble by giving
# each element of a list its own row. Before unnest(), our tibble had only 3 rows,
# and the trial_ID was stored in a neat list. After unnest() our tibble has 1,043
# (10 + 33 + 1000) rows.
...
By graphing the results for multiple samples, we have created 3 **sampling distributions**, each with a different number of observations. Looking at our sampling distributions we have some questions to answer.  
...
knitr::include_graphics("06-one-parameter/images/variance_meme.jpg")
```

Let's take a step back from all the variance. The reality is that our code needs to be *re-factored*, because we are yet again making a separate tibble for each sample size. To make comparisons easier, let's attempt to put all 3 shovel sizes in the same tibble using mapping. 

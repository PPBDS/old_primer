...
For the first question we also need to state **our prior**, which is all knowledge we know about a parameter beforehand. The prior is often denoted as $p(\theta)$, the probability distribution of a parameter $\theta$. Essentially, the prior tells us that some values are more likely than others. Without any prior information, the proportion of red beads in the urn is equally likely to be any continuous value between 0 and 1. However, we are given the prior that in a random sample of size 50, 17 beads were red, meaning that the proportion red in our sample is .34. With this knowledge we can realize that the proportion of beads in the entire urn is far more likely to be around .34 than an extreme value like .02 or .86. We will come back to the prior later in Courage to build our model.  
...
Don't get caught up by the equation. It just means that our outcome variable is chosen based on some parameter, $\theta$. If we change the value of the parameter, our outcome is likely to change as well. 
...
The total number of beads selected in ONE specific experiment with 50 sampled beads, $T_i$, is selected by taking one draw from a binomial distribution with $n = 20$ and an unknown probability $\rho_h$ of the proportion of red beads in the urn.
...
We've evaluated our data and questions and chosen a mathematical formula. Now it's time to work on our code to create a model to simulate the real world. 

...
Use the **tidyverse** package. We will also use the **dplyr** package to display summary lines.
...
3. Use the virtual shovel 1,000 and 10,000 times to examine the effect of more samples on the distribution.   
...
*How do the distributions of 33, 1,000, and 10,000 samples compare?*

Some readers may consider copying the code from above, and creating 3 new variables: One for the distribution of 33 samples, one for 1,000 samples, and one for 10,000 samples.
...
samples_10000 <- tibble(ID = 1:10000) %>% 
  mutate(shovel = map(ID, ~ slice_sample(urn, n = 50))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
  mutate(numb_beads = map_int(shovel, ~ length(.$color))) %>% 
  mutate(prop_red = numb_red / numb_beads)
...
#unnest() is a nifty function from the tidyr package, which expands a tibble by giving each element of a list its own row. Before unnest(), our tibble had only 3 rows, and the trial_ID was stored in a neat list. After unnest() our tibble has 11,033 rows. 
...
Let's examine what changes we made to our sampling distribution and why. Because our goal is to compare distributions that use the shovel different numbers of times, our new code starts with `num_trials`. If we want to compare different numbers of trials in the future, we can simply change `num_trials` instead of having to make a new tibble and variable. After we initialize the tibble with `num_trials`, we create a range of integers from 1 to our desired number of trials. Each number will represent an instance of us sampling from the virtual urn. Note how the `reps` column only has `r scales::length(num_trials)` values: `r scales::num_trials[1]`, `r scales::num_trials[2]`, and `r scales::num_trials[3]`. This will help group our tibble by number of trials later on. 
...
```{r}
larger_tibble <- multiple_sampling_distributions <- tibble(reps = num_trials) %>%
  mutate(trial_ID = map(reps, ~1:.)) %>%
  unnest(trial_ID) %>%
  # Use slice_sample()
  mutate(shovel = map(trial_ID, ~slice_sample(urn, n = shovel_size))) %>%
...
smaller_tibble <- multiple_sampling_distributions <- tibble(reps = num_trials) %>%
  mutate(trial_ID = map(reps, ~1:.)) %>%
  unnest(trial_ID) %>%
  # Use sample()

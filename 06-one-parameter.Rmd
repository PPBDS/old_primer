...
*If we get 17 red beads in a random sample of size 50 taken from a mixed urn, what proportion $p$ of the beads in the urn are red?*
...
This essence of Wisdom is determining whether or not it is solvable, as it is futile to attempt to solve an impossible problem. First, let's break down our question into its key parts. Then we can construct a Preceptor Table to determine whether or not our data is viable to answer our question. 
...
Virtually every problem boils down to attempting to answer a **question** about a **population** using **data** and **prior knowledge**. We should explicitly state all 4 before we proceed, to make sure we understand the task at hand. For example, drawing conclusions about the contents of one urn is extremely different than drawing conclusions about all urns. The concept of the population is critical.   

Let's revisit our first question: *If we get 17 red beads in a random sample of size 50 taken from a mixed urn, what proportion $p$ of the beads in the urn are red?*. We have a *question*: what proportion of the beads in the urn are red? Our question is asking us to solve for one population parameter, $p$.  The subject of the question is the *population*; in this case, the population is *a mixed urn*. "A mixed urn" is very ambiguous, and we do not even know its size, but that is what the question provides us. There is an urn out there, somewhere, for which we want to know the proportion of red beads. 
...
One of the most important terms in statistics and data science is the **Data Generating Mechanism**, or DGM. The DGM simply is how each individual outcome, $y_i$, comes into existence. In general, the DGM is often denoted as follows: 
...
- **The likelihood**, $p(y|\theta)$, represents the joint probability distribution for each value of our parameter(s) $\theta$. This is the exact same concept we addressed while discussing the distinctions of Bayesian science: because our parameters are variable, we have to create distributions for each potential value. Combining all these distributions together creates a joint distribution that is 3 dimensional when plotted. 

Careful readers may note that the equation for the likelihood, $p(y|\theta)$, appears to be very similar to the equation for the DGM, $p(y_i|\theta)$. They are indeed similar, but know that the likelihood is a distribution of all outcomes, while the DGM models selecting one individual outcome, $y_i$.  
...
In our urn scenario, obtaining the posterior involves first creating many binomial distributions for each possible population proportion. This is the likelihood, and it is a 3 dimensional model. We then select the distribution that corresponds with our data: the estimated population proportion is 34%. This is equivalent to taking a 2 dimensional slice of the 3 dimensional model. We are left with a probability distribution for our parameter, $\rho$. 

#### 4 steps for success

In Chapter \@ref(probability), we created our probability distributions "by hand". We first started our tibble with a range of possible values for our parameter $\rho$. We then took samples using each possible value to create the 3 dimensional joint distribution. Finally, we took a slice of the joint distribution based on the data we collected, to create the posterior probability distribution. This is not how professionals work. Many data scientists use the **rstanarm** package, which provides an easy interface to work with the statistical language Stan. 

**rstanarm** and Stan are appealing because they are powerful. As such, creating models using them may seem considerably more complex than our methodology before, and as we progress we will see more numbers and math. Do not fret, as our understanding of the likelihood, prior, and posterior can help us!  In this chapter we will explain our models in terms of high level Bayesian concepts, so we can understand what is going on before diving into the math in later chapters.

We will use four key steps to assist in our modeling, provided by @bayestut

> Step 1: **Specify the data model and prior**. The model is a collection of probability distributions conditional on different values for the parameters. The prior provides the priori plausibility of the different parameter values. The product of the prior and the likelihood is proportional to the posterior distribution of the parameters.

> Step 2: **Estimate the model parameters**. Bayes’ theorem is used to update beliefs about
the parameters based on knowledge obtained from observing the data. Estimating regression models in the Bayesian framework typically involves using a numerical algorithm to draw a representative sample from the posterior distribution.

> Step 3: **Check sampling quality and model fit**. Graphical and numerical checks are necessary to confirm that the sample obtained in the previous step adequately describes the posterior and that the observed data is plausible underthe model. If not, the model needs to be revised, and we might want to explore alternative model formulations.

> Step 4: **Summarize and interpret results.** We assess estimates, interpret results, and make posterior predictions.

As we create our model we will revisit these steps to ensure we understand the underlying purpose of the functions we use and objects we create, without having to understand the specific details. 

#### stan_glm()

Instead of building the joint and conditional probability distributions "by hand" as we did in Chapter \@ref(probability), we can just use `stan_glm()` from the **rstanarm** package. For now our goal is to learn how to interpret the results of `stan_glm()`. Chapter \@ref(two-parameters) will provide a more detailed explanation for the seemingly complex objects we will make. **To be clear, you do not need to fully understand this section or how this code works. This is an introduction, not a formal lesson.**

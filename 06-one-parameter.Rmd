...
<!-- I want to add 1-2 more questions to answer in Temperance. I have 2 ideas, but not sure if they have any educational value. You can go to Temperance and uncomment the code chunks to see 2 ideas I have. -->
...
The urn paradigm provides us a simple problem for which we can learn new data science techniques before we tackle more complicated problems in later chapters. For any question, we use the Cardinal Virtues to guide our thinking. While this may seem trivial at first, as our problems become more complex it will become useful to have a systematic approach to break each problem into its parts. Let's get to it.
...
This essence of Wisdom is determining whether or not a problem is even solvable. First, let's break down our question into its key parts. Then we can construct a Preceptor Table to determine whether or not our data is viable to answer our question. 

<!-- Change "centered at" to reference p. -->

#### Getting started

Virtually every problem boils down to attempting to answer a **question** about a **population** using **data**. We should explicitly state all 3 before we proceed, to make sure we understand the task at hand. 

Let's revisit our first question: *If we get 17 red beads in a random sample of size 50 taken from a mixed urn, what proportion $\rho$ of the beads in the urn are red?* We have a *question*: what proportion of the beads in the urn are red? Our question is asking us to solve for one population parameter, $\rho$.  The subject of the question is the *population*; in this case, the population is all the beads in *a mixed urn*. "A mixed urn" is very ambiguous, and we do not even know its size, but that is what the question provides us. There is an urn out there, somewhere, for which we want to know the proportion of red beads. 

To answer our question, we have some *data*: in a random sample of size 50, $17/50 = 34\%$ of beads are red. Initially, we bring no information into the problem. There is an urn out there with mixed red and white beads, and the proportion of red beads could be any value between 0 and 1. The world is dark. We are then blessed with data, and there is some light. With our sample, we know that the proportion of red beads in the whole population is much more likely to be similar to our sample proportion, 34%, than some extreme value like 2% or 80%.   

Occasionally, we will bring some information into the problem before we collect our data. For example, if we flip a fair coin, we know that in the long run there is a 50% chance of it landing on heads. Even without any data, we know *something* about the situation. Some data scientists will define one's initial beliefs going into a problem as the prior. Instead, we look to the posterior to acknowledge both our initial beliefs, as well as the data we obtain that can update those beliefs. Using the coin example again, we could flip a coin, and $11/20 = 55\%$ of flips could be red. Here our data supports our initial beliefs, and the final posterior we hope to construct will consider extreme values even less probable. 

Now let's break down our second question in a similar fashion to the first. Our *question* is: *what is the probability, using the same urn, that we will draw more than 8 red beads if we use a shovel of size 20?*. The question specifies using the "same urn", so the *population* for both questions is the same. However, do we have any *data* or *prior knowledge* to answer the second question? The reality is that we need to know the probability of selecting a red bead from the urn before we can determine the probability of selecting more than 8 red beads, and thus our second question is contingent on extracting a posterior probability distribution from the first question. As such, if we successfully pass through Wisdom and Justice, we can use the models we create in Courage to answer this question in Temperance. While that may be a mouthful at the moment, you will receive plenty of practice with the Cardinal Virtues throughout this *Primer*. 

After successfully breaking down the problem, our next step is to construct a Preceptor Table. 
...
To see if our data can answer our question, we first construct the ideal Preceptor Table. First, we should determine if we are modeling (just) for prediction or are we (also) modeling for causation? Predictive models care nothing about causation. With prediction, all we care about is forecasting Y given X on some as-yet-unseen data. There is no notion of some unseen casual effect which requires a column for both before and after the treatment. Unlike the Rubin Casual Preceptor Tables in Chapter \@ref(rubin-causal-model), our actual preceptor table is predictive and only requires one column: bead color. We can create the ideal Preceptor Table by performing the exhaustive census of the entire urn. 

```{r, echo = FALSE}
urn %>%
  slice(1:2, 200, 201, 1000) %>%
  mutate(bead_ID = as.character(bead_ID),
         color = as.character(color)) %>%
  add_row(bead_ID = "...", color = "...", .after  = 2) %>%
  add_row(bead_ID = "...", color = "...", .after  = 5) %>% 
  add_row(bead_ID = "...", color = "...", .after  = 7) %>% 
  gt() %>%
...
Where is all of our data? Well, when we take a sample of 50 beads, we only have color identifications for 50 of the unknown number of beads in the urn. The rest of the beads were not sampled and we cannot say for certain whether they are white or red.

Now, what does our actual Preceptor Table tell us, specifically, about Bead 1? Bead 2? We know the proportion of red beads in our sample is 34%. Does this mean that bead 1 has *precisely* a 34% chance of being red? As we learned in Chapter 5, this is not true! There is uncertainty.

We can only claim *for certain* that, of the 50 beads that we sampled, 34% were red. If we were making a prediction of the probability of one of our **sampled** beads being red, 34% would be the correct probability. If we were making a prediction of the probability that an **unsampled** bead was red, the answer of 34% would be incorrect, but nor is it likely to be extremely incorrect. The sample does tell us something.

The last step of Wisdom to consider is why some beads **do** get sampled, while others do not. This is a consequence of the sampling mechanism. We need to take any biases into consideration, so we can evaluate the stability, representativeness, and validity of our data in Justice. Keep in mind that just because our data can answer our question **never** means that we should use it! For example, what if our beads were mixed poorly, and most of the red beads sit at the bottom of the urn. In that case, our sampling mechanism consisting of the shovel with 50 slots after mixing the urn, would produce biased samples that would not be conclusive about the contents of the entire urn. The effects of biases can be determined later in Justice, but we need to know they exist in Wisdom. 
...
Now that we've determined that our data *could* be used, Justice asks if it *should* be used by analyzing the sampling mechanism. If we want to draw conclusions about an entire population using a small subset of its individuals, then we want to consider the stability, representativeness, and validity of our data. Skipping Justice puts us at risk of creating models with "bad data", which would lead us to incorrect or misleading conclusions.  

#### Creating the Population Table

<!-- RS: I capitalized "Data" a bunch of times to emphasize the specific category, but there is not really an underlying pattern of capitalization which needs to be fixed (but I'm too lazy to do now).  -->

The next step up from the Preceptor Table is the **Population Table**, a structure which includes three categories: the data for units we want to have (**the Preceptor Table**), the data for units which we actually have (**our actual data**), and the data for the rest of the population, not included in the data or the Preceptor Table (**the Population**). Let's construct our Population Table first, and we'll explain it's significance later. 
...
Each specific row represents one subject, which are individual beads in our urn scenario. Because there could be thousands or even millions of beads, we provide 2-3 examples for each category, and use ... to denote that there are many more subjects that we have yet to record. 

Each Population Table will have a minimum of 3 types of columns: Covariates, Time, and Outcome. We will explain these columns for each type of category (Preceptor Table, Data, and Population). 
...
- Preceptor Table: Subjects in our Preceptor Table all have a location of "Known, specific urn." This is because we want to find the proportion red of one specific urn, so ideally all of our data should come from this urn. Duh. 

- Data: Ideally, we want our data to have the same covariates as our Preceptor Table. Because we draw our sample from the exact same urn our question asks us about, the data we collect comes directly from the Preceptor Table, so they have the same location ("Known, specific urn"). This is the perfect scenario for us, but this will rarely happen in real life. For example, let's put ourselves inside the Hunger Games, where 12 districts each draw from a separate urn to select their tributes to fight to the death. What if we **want** to know the mean age of tributes selected in District 11 (The Preceptor Table), but instead we are only given data from Districts 9 and 10 (Our Data). In this scenario, the location of the Preceptor Table and the Data differs because the urns are located in two separate districts. And this could be perfectly fine! We just have to make a judgement call for whether or not the data we have is close enough to the data we need. The Population category is included just for this reason: with it, we bridge the gap between the Preceptor Table and our actual data.

- Population: In our urn scenario, the location of the Preceptor Table and data are the same, so the Population will also have the same location. Again, covariates from the Population Table and Data are rarely identical in real life. Returning to our Hunger Games example, we want data from District 11 to fill in our Preceptor Table, but we are only given data from districts 9 and 10. If we wish to draw conclusions about District 11 using Districts 9 and 10, what stops us from using Districts 1 and 2 instead? Or maybe Districts 3 and 7? **We use the Population to acknowledge the wider source we could have collected data from.** Assuming all 12 districts are similar, we really could have used data from any 2, so our location covariate for the Population should be: "All districts". On the other hand, for our urn problem there is only one urn which we could data from, so we know the location of our Population. If this still does not make sense, there will be plenty of more realistic applications of the Population later in this *Primer*. 

Population Tables always have a column for **Time**. When answering a question we must specify the time that it's being asked, because life changes over time. Finding the mean life expectancy today is different than 100 years ago. Let's break down the time column for each category. 
...
- Data: Unfortunately, we collected our sample (17 out of 50 beads are red) in the past. We ideally want to know the contents of our sample at this very instant to answer our question, but we do not have the resources to whip out an urn for every new question we have. The best we can do is acknowledge that the data we have does not fill in our Preceptor Table perfectly, because it was collected from the time of the sample in the past. Some of us may believe that making this distinction is ridiculous, as an urn does not change over time! However, it is possible that someone could have dumped some extra red beads into the urn after taking our sample. If this is the case, then our data only reflects the contents of the urn in the past, and not the urn now. 
...
Finally, Population Tables have an **Outcome**. Sometimes there will be multiple outcome columns, like in the case of Casual Effect problems where we ideally want data on a subject before and after treatment. In our urn problem, we only have one outcome: the color of each bead, either red or white.

- Preceptor Table: To find the exact proportion red in our urn, we simply divide the number of red beads in the urn by the total number of beads. However, we do not the colors of each individual bead in our urn, so we mark this information with a ?. Because our data has all the same covariates as our Preceptor Table (which is very unrealistic) we actually do know the colors of 50 beads in our Preceptor Table, but we list 2 beads that were not sampled for simplicity's sake. 
...
#### Stability, representativeness, validity

Now that we have created our Population Table, we can analyze some key issues: stability, representativeness, and validity. If we fail to stop and do this analysis and delve further into the problem, we risk reaching the end and realizing the data we have can't solve our problem in the first place. 

**Stability** involves time. Does data collected in different time periods vary? Does the data we have from the past provide insight into the population we want to learn about now, or has time changed life too much? Realistically, an urn will always be the same over time, and stability is more useful in situations involving people. However, what if someone dumps some red beads into the urn before we take our sample? Then we cannot assume validity, because the proportion of red beads in the urn the instant before the dump is different than the proportion red in the urn when we collect our data. We will assume no one is tampering with our urn, and assert stabllity across time periods. 

**Representativeness** involves our rows. More specifically, are the rows that we do have data for representative of the rows for which we do not have data? In our urn, our  data is representative of the population if the proportion of red beads in our sample is close enough to the actual proportion of red beads in the whole urn. For the sample proportion to be similar to the actual population proportion, we ideally want the data we have to be a random, unbiased selection from our population, using a considerable sample size. This rarely occurs in real life! But in the context of our problem, the sampling mechanism of using a shovel of size 50 to sample beads from a **mixed** urn should be enough to consider our sample representative of the population, and we can move on. If we didn't know the urn was mixed, there is a higher likelihood of biased sampling, and it would be harder to conclude the sample proportions are representative of the urn's actual proportion. 

**Validity** involves the columns of our data set. Is the meaning of our columns consistent across our dataset(s) and ideal Preceptor Table? In our urn scenario, does bead color in our sampled data and bead color in our Preceptor Table mean the same thing? The answer is yes, and validity can be assumed very easily. If some beads in the urn are old and the color has worn off, it may be more difficult to conclude whether the color column always means the same thing, but this is unlikely. In Chapter \@ref(two-parameters) we will analyze more realistic (and difficult) examples of stabiliity, representativeness, and validity. 

#### The DGM

The final aspect of Justice is producing a mathematical formula that can model our situation. Our models are only as good as the equations we use to make them, so choose with care. 

One of the most important terms in statistics and data science is the **Data Generating Mechanism**, or DGM. The DGM is simply how each individual outcome comes into existence. In general, the DGM is often denoted as follows: 

$$ y_i  \sim p(y_i | \theta) $$

Don't get caught up by the equation. It just means that our outcome is chosen based on some given parameter, $\theta$. If we change the value of the parameter, our outcome is likely to change as well. 

We can create a DGM for our specific urn scenario. In this *Primer*, we most frequently use either normal (Gausian) distributions or binomial distributions. In our urn we count the number of red beads selected, and do not count the white beads. This success/failure or TRUE/FALSE relationship strongly suggests we should use a binomial distribution to model our situation. We can rework the DGM equation to suit our needs: 

$$ T_i  \sim B(\rho_r, n = 50) $$

The total number of red beads selected in ONE specific experiment with 50 sampled beads, $T_i$, occurs from taking one draw from a binomial distribution with $n = 50$ observations and an unknown probability $\rho_r$ of the proportion of red beads in the urn.
...
Some non-Bayesian frameworks are concerned with the probability distribution of our observed data, but do not care much about the probability distribution for $\rho_r$ and assume it to be fixed. If $\rho_r$ is fixed, the equation above becomes one simple binomial distribution. Think of this as a standard 2 dimensional plot. 

Us Bayesians consider our observed data to be fixed. We don't consider alternate realities where our observed data is different due to sampling variation. Instead, we are concerned with the probability distribution of our parameter. In our urn scenario, $\rho_r$ is variable, so we have to create a separate binomial distribution for each possible value of $\rho_r$. Think of this as a 3 dimensional joint distribution like what we created in Section \@ref(n-models).
...
Careful readers may note that the equation for the joint distribution, $p(y|\theta)$, appears to be very similar to the equation for the DGM, $p(y_i|\theta)$. They are indeed similar, but know that the joint distribution models all outcomes, while the DGM models selecting one individual outcome, $y_i$. Also know that many data scientists refer to the joint distribution as "the likelihood".  

**The posterior**, $p(\theta|y)$, describes the relative plausibility of all parameter values, $\theta$, given some data $y$.  As soon as we incorporate data into our model, we in effect update our beliefs and create the posterior which models our uncertainty regarding a parameter. The posterior is our coveted end goal, which we can use to answer our questions. We have referenced the posterior many times before, and this definition does not change its meaning. 
...
In Chapter \@ref(probability), we first started our tibble with a range of possible values for our parameter $\rho$. We then took samples using each possible value to create the 3 dimensional joint distribution. Finally, we took a slice of the joint distribution based on the data we collected, to create the posterior distribution. This is not how professionals work. Many data scientists use the **rstanarm** package, which provides a user friendly interface to work with the statistical language Stan. 

**rstanarm** and Stan are appealing because they are powerful. Functions such as `stan_glm()` can do everything we did by hand in Chapter 5 in a few lines of code. Because we will use a professional statistical library, the objects we make will become more complex. In this chapter we will explain our models in terms of high level Bayesian concepts, and Chapter \@ref(two-parameters) will provide a more detailed explanation of the objects we will make. **To be clear, you do not need to fully understand this section or how this code works. This is an introduction, not a formal lesson.**

Again, we are attempting to answer this question: *If we get 17 red beads in a random sample of size 50 taken from a mixed urn, what proportion $\rho$ of the beads in the urn are red?*

We want to estimate the value of $\rho$, but because us Bayesians consider $\rho$ to be variable, we need to create a posterior distribution that addresses our uncertainty regarding its true value. We can easily create the posterior by using `stan_glm()`. Let's go ahead and create our model, and then view the results in a tibble. 
...
This tibble shows us 4,000 draws from the posterior distribution.`stan_glm()` simplifies all the work which we did by hand previously into one single function. We can also visualize our 4,000 draws from the posterior in a histogram:
  ...
  as_tibble() %>%
   ggplot(aes(x = `(Intercept)`)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   bins = 50) +
    labs(title = "4,000 draws from the Posterior Distribution",
         subtitle = "Why is the x axis negative? We will address this soon.",
         x = expression(rho),
         y = "Probability") + 
...
Let's examine the purpose of the arguments used within `stan_glm()` in terms of high level Bayesian concepts. 

**The joint distribution**: The `family` and `formula` arguments relate to the joint distribution, also known as the likelihood by some. In Justice, we created the following model for our sampling mechanism:

$$ T_i  \sim B(\rho_r, n = 50) $$

This is a binomial distribution. In `stan_glm()` we denote this with `family = binomial`. In addition to the type of the distribution, we also need to analyze the outcome and covariates involved. The outcome is the quantity we are measuring, in this case the total number of red beads in our sample, $T_i$.  Covariates are predictor variables that help forecast our outcome. Because we only measure how many red beads are in the sample (the outcome), we have no covariates which we can use to make our model more accurate. Because we have no predictors, we use the argument `formula = red ~ 1`, which means that we only model the outcome without the consideration of any covariates. 

**The data**: We are required to pass data into the `data` argument in order for `stan_glm()` to "take a slice" of the joint distribution and estimate the posterior. Because we are measuring the total number of red beads in a sample, $T_i$, we pass in data in a binomial format: the 1's represent the number of successes (red beads drawn), and the 0's represent the number of failures (white beads drawn). As such, we pass a tibble with 17 red beads and 33 white beads into `data`. 

**Other arguments**: we use `refresh = 0` to suppress the behavior of printing to the console, and `seed = 10` to make produce the same output every time we run the code. 

 Once we have the `fit_1` object, it is easy to answer two sorts of questions: the posterior probability distribution for $\rho$ and predictions for new draws from the urn. The key new functions are `posterior_epred()` for the former and `posterior_predict()` for the latter.
 
#### Answering the questions
 
There are some questions left to be answered. Let's attempt to answer them here. 

> Why is the x axis of the posterior distribution negative? Shouldn't all values be between 0 and 1?

The x axis of our posterior is negative because for binomial models, `stan_glm()` chooses to use the "logit" function to convert our outcomes to log probabilities. We can use `posterior_epred()` to find the **e**xpected **pred**iction of our posterior without the logit function. Know that we do not just use `posterior_epred()` to undo transformations. We will use it many more times throughout this *Primer* to create the expected distribution based off some covariate.  Let's recreate our posterior using `posterior_epred()`:
...
Because we do not wish to create our distribution conditional on some covariate, we want to pass in a tibble with 1 row into the `newdata` argument. An empty tibble has 0 rows, so we instead pass in a tibble with some junk data. `constant = 1` is completely meaningless. We can verify that the posterior is the same by plotting it. Hopefully it should have a similar shape, but all values should be between 0 and 1. 
...
ppd_for_p %>% 
  ggplot(aes(x = p)) +
    ...
    labs(title = "Posterior Probability Distribution",
         subtitle = "Distribution shape is similar to earlier, but the proportion red is centered at .34",
...
Now that our posterior probability distribution is correctly centered, we have successfully created the posterior distribution and can finally answer the question we started the chapter with: *If we get 17 red beads in a random sample of size 50 taken from a mixed urn, what proportion $\rho$ of the beads in the urn are red?*
...
Now that we have our posterior distribution for the proportion $\rho$ of red beads in the urn, we can use that object to forecast other outcomes, outcomes which we have not yet observed. We can now answer the question from the start of the Chapter: 
...
**There is approximately a `r round(100 *sum(ppd_reds_in_20$total > 8)/length(ppd_reds_in_20$total))`% chance that we will draw more than 8 red beads.**
...
<!-- RS: Can we ask a question like: If we draw 22 red beads out of 50 from a new urn, how certain are we that the two urns have the same proportion red? 

I would create 2 posteriors and calculate the overlapping area, and then finally do (1 - overlapping area) to calculate the percent certainty that the two urns have a different proportion red (we cannot answer whether or not they have the same). I am not sure a.) if that is an answerable question b.) if the way I explained above is the correct way to answer the question and c.) even if we can answer the question, should we even answer it? Does knowing the percent even help us?  I saw earlier comment that "we can never have enough finding the area under the curve stuff", so just want to give more practice. Code below illustrates what I'm thinking.  -->

<!-- ```{r} -->
<!-- fit_3 <- stan_glm(formula = red ~ 1, -->
<!--                   data = tibble(red = c(rep(1, 22), -->
<!--                                         rep(0, 28))), -->
<!--                   family = binomial, -->
<!--                   refresh = 0, -->
<!--                   seed = 10) -->

<!-- ppd3<- posterior_epred(fit_3, -->
<!--                 newdata = tibble(constant = 1)) %>% -->
<!--   as.vector() -->


<!-- ppd1 <- posterior_epred(fit_1, -->
<!--                 newdata = tibble(constant = 1)) %>% -->
<!--   as.vector() -->

<!-- #Next 2 lines produce the same result, because the number of overlapping data points are the same for both vectors, and there are 4,000 rows in each vector. -->

<!-- 1 - ((length(ppd3[ppd3 < ppd1]) + length(ppd1[ppd3 < ppd1]))/length(ppd3)) -->
<!-- 1 - (2*length(ppd1[ppd3 < ppd1])/4000) -->


<!-- # We are 71.7% sure that the two urns do not have the same proportion red? -->

<!-- ppd_for_p %>% -->
<!--   ggplot(aes(x = p)) + -->
<!--   geom_histogram(data = ppd3 %>% as_tibble() %>% rename(p = value), -->
<!--                  aes(y = after_stat(count/sum(count))), bins = 50, fill = "blue", alpha = .5) + -->
<!--     geom_histogram(aes(y = after_stat(count/sum(count)), fill = "blue", alpha = .5), -->
<!--                    bins = 50) + -->
<!--     labs(title = "Posterior Probability Distribution", -->
<!--          subtitle = "Distribution shape is the same as earlier, but the proportion red is centered at .34", -->
<!--          x = "Proportion p of Red Beads in Urn", -->
<!--          y = "Probability") + -->
...
<!-- RS: This code below serves to illustrate that the more data we have, the more refined our posterior becomes. Use data from real sampling activity (33 trials). Again, not sure if there is any value to this.  -->

<!-- ```{r} -->
<!-- newdata <- tactile_sample_urn %>% -->
<!--   select(red_beads) %>% -->
<!--   mutate(red = map(red_beads, ~ c(rep(1, .), rep(0, 50-.)))) %>% -->
<!--   select(red) %>% -->
<!--   unnest(red) -->


<!-- fit_4 <- stan_glm(family = binomial, -->
<!--                   seed = 10, -->
<!--                   refresh = 0, -->
<!--                   formula = red ~ 1, -->
<!--                   data = newdata) -->

<!-- ppd4 <- posterior_epred(fit_4, newdata = tibble(constant = 1)) %>% -->
<!--   as_tibble() %>% -->
<!--   rename(p = `1`) -->
...
<!--                    bins = 50) + -->
<!--     labs(title = "Posterior Probability Distribution", -->
<!--          subtitle = "Much smaller range of values on the x axis than before", -->
<!--          x = "Proportion p of Red Beads in Urn", -->
<!--          y = "Probability") + -->
...
Although the process was long, we have successfully gone through the cardinal virtues and answered our questions like real data scientists. In this section we will address several pitfalls to avoid. 
...
<!-- RS: Going to use this section to address how we should not conclude that our models are the truth. Also, explain the purpose of the cardinal virtues, because it would suck to do all this work and realize we made incorrect assumptions and it is all useless! -->

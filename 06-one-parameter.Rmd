...
1. **Refactor** our code to allow us to sample as many times as we want. 
...
```{r, echo = FALSE, fig.cap = "Although it seems like more work intially, refactoring our code will help in the longrun."}
...
Now Let's attempt to refactor our code to create one tibble that includes the sampling distributions for any number of trials we choose. First, create the foundation for our tibble. 
...
Let's use our newly refactored code to replicate the tactile sampling activity in a virtual format. We'll save these results in a data frame called `virtual_samples`. 
...
The advantage of modern technology is that we can use virtual simulation as many times as we choose! No longer do we have to have to recruit groups of friends to sample, which would be especially troublesome as we increase the number of samples. Let's review our refactored tibble to compare the differences between 10, 33, and 1000 trials.    
...
Along with creating the `multiple_sample_distributions` tibble with our distributions for each number of trials, we will also create a tibble called `summary_lines` that will contain the mean $\hat{p}$ across each value of `reps`. We can use the `summary_lines` tibble to help us visualize our sampling distributions: 
...
Now we can combine the `multiple_sampling_distributions` and `summary_lines` tibbles to compare the sampling distributions for 10, 33, and 1,000 trials. 
...
<!-- Should we discuss the normal distribution here? -->

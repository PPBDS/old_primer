...
<!-- RS: New Chapter 6 Layout Proposal:  -->
<!-- 6.1: Real sampling activity -->
<!-- - Keep the same as now, but possibly introduce some of the vocab.  -->

<!-- 6.2: Increasing trials -->
<!-- -Goals: Goal is to try and replicate the in real life sampling activity using virtual simulation. Use tools like mapping and functions to make our life easier. Bring up the point that while sampling in real life it can be time consuming to perform many trials. So, how would doing 33 trials in real life compare to doing 1,000 trials virtually? The answer is that our distribution becomes more like the normal distribution as we experience more extreme results as we increase the number of trials.  -->

<!-- - 6.2.1: Mapping: Mapping refresher. How does it work? Why is it useful? map v. map_int()? Refer readers back to Ch. 5 if they need additional help.  -->
<!-- - 6.2.2: Virtual sampling:  First create the urn (Maybe pick a random number for the number of red and white beads in the urn?). Then create our sampling tibble step by step, explaining all relevant functions. Compare results to real life sampling activity. At the end allude to the fact that functions would be helpful to allow us to sample as many times as we want instead of having to manually input the number of times we want to sample.  -->
<!-- - 6.2.3: Functions: What is a function? What is the notation? How can we use functions in the situation above? -->
<!-- - 6.2.4: Effect of more trials: Show histograms from different numbers of trials. Point out the development of tails as trials increase, because you are more likely to find outlier trials when you have more trials. Finally create gganim of mean and standard deviation to show effect of more trials and lead to the normal approximaiton.  -->
<!-- - 6.2.5: The normal approximation: Present mathematical equation for the normal approximation, and explain the properties of the normal distribution. Plot a graph of the normal distribution and virtual sampling side by side.  -->
...
<!-- - 6.3.3: Graphical explanation: Compare histograms of different sample sizes. (Try and make a gganim.) Display summary lines of mean and standard deviation.  -->
...
<!-- - 6.4.1: The Questions: In a well mixed urn with an unknown number of red and white beads, we get 17 red beads in a random sample of size 50. (Should we use data on more than 1 sample?) -->

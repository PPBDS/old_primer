...
Virtually every problem boils down to attempting to answer a **question** about a **population** using the **prior**. We should explicitly state all 3 before we proceed, to make sure we understand the task at hand. For example, drawing conclusions about the contents of one urn is extremely different than drawing conclusions about all urns. The concept of the population is critical.   
...
For the first question we also need to state **our prior**, which is all knowledge we know about a parameter beforehand. The prior is often denoted as $p(\theta)$, the probability distribution of a parameter $\theta$. Essentially, the prior tells us that some values are more likely than others. Without any prior information, the proportion of red beads in the urn is equally likely to be any continuous value between 0 and 1. However, we are given the prior that in a random sample of size 50, $17/50 = 34\%$ of beads are red. With this knowledge we can realize that the proportion of beads in the entire urn is far more likely to be around .34 than an extreme value like .02 or .86. We will come back to the prior later in Courage to build our model.  

Now let's break down our second question in a similar fashion to the first. Our *question* is: *what is the probability, using the same urn, that we will draw more than 8 red beads if we use a shovel of size 20?*. The question specifies using the "same urn", so the *population* for both questions is the same. However, what is the *prior* for the second question? The reality is that we need to know the probability of selecting a red bead from the urn before we can determine the probability of selecting more than 8 red beads, and thus our second question is contingent on extracting a posterior probability distribution from the first question. As such, if we successfully pass through Wisdom and Justice, we can use the models we create in Courage to answer this question in Temperance. While that may be a mouthful at the moment, you will receive plenty of practice with the Cardinal Virtues throughout this *Primer*. 
...
- **The prior**, $p(\theta)$, is the probability distribution that describes our uncertainty surrounding our parameter(s) $\theta$. We create the prior from any information we have going into the problem, and the more descriptive the information the more descriptive the prior. 

In Wisdom, we stated that the prior for our urn problem is knowing that in a random sample of size 50, $17/50 = 34\%$ of beads are red. This information is useful, as without it the proportion of red beads in the urn could be any value between 0 and 1. However, we still need to acknowledge that due to sampling variation, we could have sampled a different value for $\rho$. This is why the prior is a distribution: we know that $\rho$ is more likely to be around .34 than extreme values, but there is a continuous range of values that $\rho$ *could* be. 

- **The posterior**, $p(y|\theta)$, is the result of Bayes' theorem and is our coveted end goal. We have discussed the posterior many times throughout this primer

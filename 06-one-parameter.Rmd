...
```
<!-- RS: The x axis seems to not be uniform, as  a proportion out of 25 is different than a proportion out of 50 than a proportion out of 100. Maybe use geom_density the graphs? Or change bin widths? -->
...
In the last section, we ran more or less the same code three times, but with different sizes for our shovel: 25, 50, and 100. We can use `expand_grid()` and a new variable, `shovel_size`, to create a tibble which will organize our results.  


```{r}
expand_grid(ID = c(1:3), shovel_size = c(25, 50, 100))
```
...
We changed the second line to use `shovel_size` rather than `ID` as the mapping variable since we can no longer hard code the shovel size into the call to `slice_sample()`. Expand to 1,000 simulations for each value of `shovel_size` and finish with a calculation of standard deviation.
...
This is, approximately, the same result as we saw above, but calculated without so much code duplication.
...
<!-- RS: I agree with whoever wrote the above comment in theory, but not completely sure about what the execution looks like. I think it is important to tie the distribution into what the standard deviation tells us about our data and the shape of our graph.  -->

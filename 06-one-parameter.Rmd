...
<!-- Chat with Yuhan. Map functions and list-columns are taught in chapter 5, both the Primer and the tutorial. Take look. Ensure that Chapter 5 covers clearly anything you use. Done -->
...
<!-- Why doesn't the percentage red match between the physical urn and the virtual urn? Please explain this, I'm confused. -->

<!-- RS: I want to add 1-2 more questions to answer in Temperance. I have 2 ideas, but not sure if they have any educational value. You can go to Temperance and uncomment the code chunks to see 2 ideas I have. -->

<!-- RS: Should we walk through solving the problem without rstanarm very quickly (like one or two code chunks)? Seems kind of rude to make people learn all these joint distributions in Ch.5 and then never use it again.  -->

<!-- RS: In some paragraphs we say that it is impossible to conduct a census because we don't have access to the urn, in others like this we do have access and can complete the census. Which is it? -->
...
<!-- RS: I changed all p's to rho's but not 100% sure if that's what was desired.-->
...
<!-- RS: Delete ID column? Not needed to solve the problem, but may be useful in understanding the Preceptor Table.   -->
...
The last step of Wisdom to consider is why some beads **do** get sampled, while others do not. This is a consequence of the sampling mechanism. We need to take any biases into consideration, so we can evaluate the validity and representatives of our data in Justice. Keep in mind that just because our data can answer our question **never** means that we should use it! For example, what if our beads were mixed poorly, and most of the red beads sit at the bottom of the urn. In that case, our sampling mechanism consisting of the shovel with 50 slots after mixing the urn, would produce biased samples that would not be conclusive about the contents of the entire urn. The effects of biases can be determined later in Justice, but we need to know they exist in Wisdom. 
...
tibble(source = c("Preceptor Table", "Preceptor Table", "...",
  ...
  cols_label(source = md("Source"),
             ...
             color = md("Color")) %>%
  text_transform(
    locations = cells_body(columns = 2, rows = c(1:2, 4:6)), 
    fn = function(x) local_image(filename = "06-one-parameter/images/sampling_bowl_1.jpg", height = 50)) %>%
  tab_style(cell_borders(sides = "right"),
...
Overall, the Population Table bridges the gap between the information we need to easily answer our question (The Preceptor Table), and the data we have. The subjects of the Preceptor Table and the subjects in our data often vary in 2 main ways: 

**Time**: Life changes over time. The mean life expectancy today is different than 100 years ago. The Population Table takes this into consideration by adding a time column. 

**Demographics**: Everything has demographics. The beads in our urn have a color. People have an age, race, height, weight, location where they live ... you get the point. When we construct a Preceptor Table to answer our question, we must select several demographics that we **want** all of our subjects to have. To answer our questions, we in theory should collect data from subjects with our desired demographics. However, life is tough, and the data we get is not always ideal. Let's use the hunger games as an example. 

If you are unfamiliar with the Hunger Games, 12 districts each select tributes to compete in a brutal fight to the death. Tributes are selected by drawing 

<!-- RS: Kind of confused about the Population Table. a.)  How is the unknown Population different than the Preceptor Table? b.) should we include bead ID? -->

#### Stability, representativeness, validity

**Stability** involves location and time. If we collect our data in a different time period will it vary? Does the data we have from the past provide insight into the population we want to learn about now, or has time changed life too much? Realistically, an urn will always be the same over time, and stability is more useful in situations involving people. However, what if someone dumps some red beads into the urn before we take our sample? Then we cannot assume validity, because the proportion of red beads in the urn the instant before the dump is different than the proportion red in the urn when we collect our data. We will assume no one is tampering with our urn, and assert stabllity across time periods. 
...
It is essential to understand the joint distribution and the posterior, two concepts Bayesians use to solve problems. We will provide quick definitions of them both here, including statistical notation which may be helpful to some. 
...
This is a binomial distribution. In `stan_glm()` we denote this with `family = binomial`. In addition to the type of the distribution, we also need to analyze the outcome and predictor variables involved. The outcome is the quantity we are measuring, in this case the total number of red beads in our sample, $T_i$.  Covariates are continuous predictor variables that help forecast our outcome. For example, age is a covariate that effects the outcome of someone's blood pressure. Because we only measure how many red beads are in the sample (the outcome), we have no covariates which we can use to make our model more accurate. Because we have no predictors, we use the argument `formula = red ~ 1`, which means that we only model the outcome without the consideration of any covariates. 
...
<!-- RS: In some paragraphs we say that it is impossible to conduct a cencus because we don't have access to the urn, in others like this we do have access and can complete the cencus. Which is it? -->
...
#### Ansewering the questions (again)

...
<!-- - 6.3.1: Comparing sample sizes: Create graph of 1000 p values for several sample sizes. -->
<!-- - 6.3.2: Standard error: Definition of standard error, using both the equation as well as explaining its meaning without all the math.  -->
<!-- - 6.3.3: Graphical explanation: Compare histograms of different sample sizes. (Try and make a gganim.) Display summary lines of mean and standard deviation.  Overlay density distributions over the histograms. -->
<!-- - 6.3.4: SE of n sized sample: Create graph of standard deviation v. sample size, and connection to SE. Try and overlay or put side by side the cloud of points for each sample size and the standard error for each sample size.  -->
...
`r scales::comma(nrow(urn))` beads. The first variable `ID` is used as an *identification variable*. None of the beads in the actual urn are marked with numbers, but the `ID` column will be relevant later on. The second variable `color` indicates whether a particular virtual bead is red or white. 
...
# 1) and taking a sample of 50 beads. Note that the ID in this tibble has
# nothing to do with the ID variable in the urn we made previously. The former refers to each sample we take from the urn. The latter refers to each individual bead in the urn.

tibble(ID = 1) %>% 
  mutate(shovel = map(ID, ~ slice_sample(urn, n = 50)))
```

<!-- RS: Less ambiguous variable naming to try and not confuse readers? Maybe instead of initializing each tibble with `ID`, we could use bead_ID, trial_ID, shovel_ID etc. -->
...
There are two levels. There is one row in the tibble for each sample. So far, we have only drawn one sample. Within each row, there is a second level, the tibble which is the sample. That tibble has two variables: `ID` and `color`. This is the advantage to using `slice_sample()`, because it selects all columns of our urn, whereas `sample()` can only sample from a single column. While the bead `ID` may be irrelevant in our urn scenario, with other problems it could be very useful to have additional data about each individual.
...
# To count the number of red beads in each shovel, we can use a lesser known property of the sum() function: By passing in a comparison expression, sum() will count the number of occurrences within a vector. In this case, we count the total number occurrences of the word red in the color column of shovel. 
...
<!-- RS: This above part kind of annoys me tbh. -->
```{r}
tibble(ID = 1) %>% 
  mutate(shovel = map(ID, ~ slice_sample(urn, n = 50))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
...
Third, calculate the proportion red, and view the final output for our single sample:
...
### Using the virtual shovel multiple times {#shovel-multiple-times}

Let's look back on the concept of **sampling variation** that we discussed previously. Because the number of beads drawn will vary from sample to sample, it is not good enough to estimate the proportion of beads in the whole urn from one measly sample. We need to use the virtual shovel multiple times! 

The agenda for this section is as follows: 

1. **Re-factor** our code to allow us to sample as many times as we want. 
2. Use our new code to replicate the tactile sampling exercise in Section \@ref(sampling-activity), but virtually! We can then compare the results from real life and our virtual simulation. 
3. Use the virtual shovel 1,000 times to examine the effect of more samples on the distribution.   

#### Re-factoring our code {#refactoring-one-parameter}

In order to use our virtual shovel different numbers of times and compare the results, lets give ourselves a question to answer: 

*How do the distributions of 10, 33, and 1,000 samples compare?*

Some readers may consider copying the code from above, and creating 3 new variables: One for the distribution of 10 samples, 33 samples, and one for 1,000 samples.

```{r}
samples_10 <- tibble(ID = 1:10) %>% 
  mutate(shovel = map(ID, ~ slice_sample(urn, n = 50))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
  mutate(numb_beads = map_int(shovel, ~ length(.$color))) %>% 
  mutate(prop_red = numb_red / numb_beads)

samples_33 <- tibble(ID = 1:33) %>% 
  mutate(shovel = map(ID, ~ slice_sample(urn, n = 50))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
  mutate(numb_beads = map_int(shovel, ~ length(.$color))) %>% 
  mutate(prop_red = numb_red / numb_beads)

samples_1000 <- tibble(ID = 1:1000) %>% 
  mutate(shovel = map(ID, ~ slice_sample(urn, n = 50))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
  mutate(numb_beads = map_int(shovel, ~ length(.$color))) %>% 
  mutate(prop_red = numb_red / numb_beads)
...
While some people may be satisfied with this solution, it is not good enough for us! What if, for example, we wanted to compare the distributions from using the shovel for every integer value between 1 and 100. If we used our previous code, we would have to make 100 variables, which would be very time consuming to do. There must be a better way! 

```{r, echo = FALSE, fig.cap = "Although it seems like more work intially, re-factoring our code will help in the longrun."}
knitr::include_graphics("06-one-parameter/images/code_refactor_meme.jpeg")
```

As data scientists it is important to stop and look back on our code from time to time. First we need to look at our end goal: To compare using the virtual shovel different numbers of times. Then we look at our code. Does having a tibble for each sampling distribution make sense? Will it be easy to work with later? The answer is no. Instead, our goal should be to create one single tibble which we can group by number of samples. The process of restructuring our code to make it cleaner, simpler, and more versatile is called **re-factoring**, and any *good* developer will do it from time to time. 

Now Let's attempt to re-factor our code to create one tibble that includes the sampling distributions for any number of trials we choose. First, create the foundation for our tibble. 

```{r}
num_trials <- c(10, 33, 1000)

multiple_sampling_distributions <- tibble(reps = num_trials) %>%
  
#After we create a tibble with 3 rows, one for each number of trials, we create a range from 1 to . , where . passes in the number or trials using mapping. 
  
  mutate(trial_ID = map(reps, ~1:.)) %>%

#unnest() is a nifty function from the tidyr package, which expands a tibble by giving each element of a list its own row. Before unnest(), our tibble had only 3 rows, and the trial_ID was stored in a neat list. After unnest() our tibble has 1,043 rows. 
  
  unnest(trial_ID)

multiple_sampling_distributions
```

Let's examine what changes we made to our sampling distribution and why. Because our goal is to compare distributions that use the shovel different numbers of times, our new code starts with `num_trials`. If we want to compare different numbers of trials in the future, we can simply change `num_trials` instead of having to make a new tibble and variable. After we initialize the tibble, we create a range of integers from 1 to our desired number of trials. Each number will represent an instance of us sampling from the virtual urn. Note how the `reps` column only has `r length(num_trials)` values: `r num_trials[1]`, `r num_trials[2]`, and `r num_trials[3]`. This will help group our tibble by number of trials later on. 

For each trial we will take a sample using `slice_sample()`, and then calculate the number of red beads in the sample and $\hat{p}$. This is the same code from when we sampled from the virtual urn a single time earlier on. 
...
multiple_sampling_distributions <- tibble(reps = num_trials) %>%
  mutate(trial_ID = map(reps, ~1:.)) %>%
  unnest(trial_ID) %>%
  mutate(shovel = map(trial_ID, ~slice_sample(urn, n = shovel_size))) %>%
  mutate(numb_red = map_int(shovel, ~sum(.$color == "red"))) %>%
...
Another way to re-factor our code would be to use functions. To reduce complexity we will not use functions, but additional information about what they are and how to use them can be found in Section \@ref(functions). 

<!-- RS: Include one code chunk creating a function to show what they are or should we not show them whatsoever? -->
Before we go onto using our new tibble to compare the effect of increasing the number of trials, there are still two re-factoring topics left to discuss: the first being *reducing ambiguity*. If we want to present our findings to other data scientists, or ask for help online, it is important that others can understand our code. To increase understanding and reduce ambiguity, we should put some effort into creating variable names that others can easily understand. We should also avoid complicated nested functions, as they are both difficult to debug and hard to understand. Finally, we should include lots of comments in our code that can be used to document our thought process, which will also help outsiders audit our code. The more comments the better!

The final topic of re-factoring left to discuss is *efficiency*. Because most of us run R Studio on our local machines, each R variable is stored using memory on our computers. As we add more columns or more complex objects to our tibbles, they will take up more memory. While our urn simulation takes up a relatively small amount of storage and we are not overly concerned with optimizing tibble size, it never hurts to practice making our tibbles more efficient. This will become especially handy when working with large data sets with thousands of rows and dozens of columns. 

In our urn exercise, the largest hindrance to efficiency are our chosen datatypes. Although we haven't really cared thus far, using `slice_sample()` will store tibbles inside our tibble, while `sample()` will store a list. We chose `slice_sample()` primarily because it can be useful with real data sets to select multiple columns, while `sample()` does not have that functionality. However, that extra data comes at a cost! A tibble with nothing in it takes up `r object.size(tibble())` of memory, so generating an individual sample tibble for each of the `r scales::comma(nrow(multiple_sampling_distributions))` rows in our tibble will make our object substantially larger. It is certainly worthwhile to read (or at least skim) this [article about memory in R](http://adv-r.had.co.nz/memory.html) to make sure that we understand the implications of storing complex objects within tibbles, so we can make the most out of our future list-column mapping endeavors. 

R makes it easy to analyze the size of our objects using the `object.size()` function. Let's compare the effect of using `slice_sample()` versus using `sample()`.

```{r}
larger_tibble <- tibble(reps = num_trials) %>%
  mutate(trial_ID = map(reps, ~1:.)) %>%
  unnest(trial_ID) %>%
  # Use slice_sample()
  mutate(shovel = map(trial_ID, ~slice_sample(urn, n = shovel_size))) %>%
  mutate(numb_red = map_int(shovel, ~sum(.$color == "red"))) %>%
  mutate(numb_beads = shovel_size) %>%
  mutate(prop_red = numb_red/shovel_size)

smaller_tibble <- tibble(reps = num_trials) %>%
  mutate(trial_ID = map(reps, ~1:.)) %>%
  unnest(trial_ID) %>%
  # Use sample()
  mutate(shovel = map(trial_ID, ~sample(urn$color, size = shovel_size))) %>%
  mutate(numb_red = map_int(shovel, ~sum(. == "red"))) %>%
  mutate(prop_red = numb_red/shovel_size)

paste("Larger tibble size: ",format(object.size(larger_tibble), units = "auto"))
paste("Smaller tibble size: ",format(object.size(smaller_tibble), units = "auto"))
```

The results are that using `slice_sample()` causes our tibble to be over 2 times larger than using `sample()`. Considering that both tibbles are relatively small, the increased size caused by `slice_sample()` is insignificant, so it is perfectly fine to continue to use it over `sample()`! Again, we only really care about the effects of inefficiency and memory intensive datatypes when our data sets become much larger, so let's not worry too much about being the most efficient in this exercise. 

#### Using the virtual shovel 33 times {#shovel-33-times}
<!-- RS: Reason why some titles do not have specific URL thingys? -->
...
Let's use our newly re-factored code to replicate the tactile sampling activity in a virtual format. We'll save these results in a data frame called `virtual_samples`. 
<!-- RS: Old code for 33 samples: -->
...
virtual_samples <- multiple_sampling_distributions %>%
  filter(reps == 33)
...
<!-- RS: Can we decide on a boundary size? I feel like it's different for every graph. -->
...
facet_compare <- bind_rows(virtual_samples %>% 
  ...
  # Set numb_red = red_beads to make the column names in both tibbles the same. 
                              ...
                              select(ID, numb_red = red_beads, prop_red) %>% 
  ...
  # the boundary argument makes sure that at least one bin borders .4, because we have the unusual insight in knowing that the population proportion in the urn is .4 (600 white beads and 400 red beads). Setting the color to white changes the border color, making the distribution easier to visualize. 
...
The advantage of modern technology is that we can use virtual simulation as many times as we choose! No longer do we have to have to recruit groups of friends to sample, which would be especially troublesome as we increase the number of samples. Let's review our re-factored tibble to compare the differences between `r num_trials[1]`, `r num_trials[2]`, and `r num_trials[3]` trials.    
...
summary_lines <- multiple_sampling_distributions %>%
  group_by(reps) %>%
...
Remember: There are `r length(num_trials)` values for `reps`, one for each number of trials we want to compare. We then expand our tibble to have a `trial_ID` from 1 to our desired number of reps. Each row now represents one instance of a sample from our urn. Finally we compute the number of red beads in each sample and $\hat{p}$.

Along with creating the `multiple_sample_distributions` tibble with our distributions for each number of trials, we also create a tibble called `summary_lines` that contains the mean $\hat{p}$ across each value of `reps`.

Now we can combine the `multiple_sampling_distributions` and `summary_lines` tibbles to compare the sampling distributions for `r num_trials[1]`, `r num_trials[2]`, and `r num_trials[3]` trials. 

```{r}

# stat(density) reformats the y axis to a density distribution.
# We divide the density by 50 in order to make binwidth*density = 1,
# or 100% of the population.

# geom_vline() is created for each instance in our facet_wrap, and 
# uses the mean to determine the x location for the vertical line. 

multiple_sampling_distributions %>%
  ggplot(aes(x = prop_red)) +
  geom_histogram(binwidth = .02, aes(y = stat(density)/50), boundary = .4, color = "white") +
  facet_wrap(~reps) + 
  geom_vline(data = summary_lines, aes(xintercept = mean, color = "red")) +
  labs(title = "Using the virtual shovel 10, 33, and 1000 times.",
       y = "Percent of Occurances", 
       x = expression(hat(p)),
       color = "mean") + 
   scale_y_continuous(labels = scales::percent_format()) 
```

First, why the empty spaces among the bars? Recall that, with only 50 beads, there are only 51 possible values for $\hat{p}$: 0, 0.02, 0.04, ..., 0.98, 1. A value of 0.31 or 0.47 is impossible, hence the gaps.

Some other interesting trends appear. First, notice how the sampling distribution for `r num_trials[1]` trials is very tall, while `r num_trials[3]` trials is shorter. This occurs because as we increase the number of trials, tails develop on our distribution as extreme $\hat{p}$ values are more likely to occur. The symmetric, bell-shaped distribution shown in the histogram of `r num_trials[3]` trials is well approximated by the *normal distribution*. 

The most frequently occurring proportions of red beads occur, again, between 35% and 45%. But 35% to 40% is a wide range! We can use the `summary_lines` tibble to help better quantify this trend. Each red line displays the mean sample proportion, and as we increase the number of trials, the red line becomes closer and closer to the actual value of the sample proportion. We can also see this analytically:

```{r, echo = FALSE}
summary_lines
```

This is an important property of sampling. For problems involving proportion (p) or the mean ($\mu$), we can take repeated samples, and know that the mean of our sampling distribution will be approximately equal to the unknown population parameter we want to evaluate. However, it is *very* important to know that for the above to be true, we must take *random* samples *directly from the population*. 

Now that we've examined the effect if increasing the number of trials, we will now change our virtual shovel size. 
...
```{r}
# Within slice_sample(), n = 25 represents our shovel of size 25.  
...
> The concept of variation will be important for the following section, so it's important to have a solid understanding. If you are still confused about this concept, let's turn to a simpler exercise: The coin flip.
>
> If you flip a coin 10 times, you would expect to flip around 5 heads, but if you were lucky you could flip 8 or 9. If one trial represents flipping the coin 10 times, there will be considerable variation between trials. 
>
> On the other hand, if one trial consisted of flipping a coin 10,000 times, you would expect to flip close to 5,000 heads every single trial. With a larger sample size there is less variation. 
...
Let's take a step back for a minute. The reality is that our code needs to be *re-factored*, because we are yet again making a separate tibble for each sample size. To make comparisons easier, let's attempt to put all 3 shovel sizes in the same tibble using mapping. 
...
```
This process of creating this tibble is very similar to the creation of our re-factored tibble in Section \@ref(refactoring-one-parameter) except for a few differences. We initialized both tibbles with our desired number of trials, but this time we map the shovel size to the trial number. Using this mapping, we will use 3 different sized shovels for each of our 1,000 trials. Thus, our tibble will have 3 $\cdot$ 1,000, or 3,000 rows. Finally, we use mapping within `slice_sample()` to pass in the shovel size. 
...
```{r}
expand_grid(ID = c(1:1000), shovel_size = c(25, 50, 100)) %>% 
  mutate(shovel = map(shovel_size, ~ slice_sample(urn, n = .))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
  mutate(numb_beads = map_int(shovel, ~ length(.$color))) %>% 
  mutate(prop_red = numb_red / numb_beads) %>% 
...
```{r, cache = TRUE}
shovels_100 <- expand_grid(ID = c(1:1000), shovel_size = c(1:100)) %>% 
  mutate(shovel = map(shovel_size, ~ slice_sample(urn, n = .))) %>% 
  mutate(numb_red = map_int(shovel, ~ sum(.$color == "red"))) %>% 
  mutate(numb_beads = map_int(shovel, ~ length(.$color))) %>% 
  mutate(prop_red = numb_red / numb_beads) %>% 
...
<!-- RS: Not correct, standard error applies to population proportions.  -->

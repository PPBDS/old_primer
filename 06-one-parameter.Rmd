...
Finally, Population Tables have an **Outcome**. Sometimes there will be multiple outcome columns, like in the case of Casual Effect problems where we ideally want data on a subject before and after treatment. In our urn problem, we only two outcomes: bead ID and bead color. 

- Preceptor Table: To find the exact proportion red in our urn, we simply divide the number of red beads in the urn by the total number of beads. However, we do not the colors (or IDs) of each individual bead in our urn, so we mark this information with a ?. Because our data has all the same covariates as our Preceptor Table (which is very unrealistic) we actually do know the colors and IDs of 50 beads in our Preceptor Table, but we list 2 beads that were not sampled for simplicity's sake. 
...
We have successfully created the posterior distribution and can finally answer the question we started the chapter with: *If we get 17 red beads in a random sample of size 50 taken from a mixed urn, what proportion $\rho$ of the beads in the urn are red?*
...
The process of the cardinal virtues may seem tedious, but we use them for good reason. 

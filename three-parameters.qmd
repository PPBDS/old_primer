...
<!-- DK: Do the major surgery we discussed in age ~ party. Put addendum approach as main approach. DONE - addendum approach implemented in age~party & att_end~treatment and income~age & liberal~income both already have the addendum approach-->

<!-- add plots: DONE added plots to the data analysis sections -->
...
trains |> 
  ggplot(aes(x = age, fill = party)) +
    geom_histogram(aes(y = after_stat(count/sum(count))),
                   ...
                   bins = 100, 
                   position = "identity") +
    labs(title = "Probability for Average Age",
...
Justice concerns four topics: stability, representativeness, unconfoundedness and the mathematical structure of the data generating mechanism (DGM).
...
<!-- Lots to clean up about this plot! (remove plot below) -->
...
Let's take a different approach and instead of parameterizing this model without an intercept, we could have used one. In that case, the math would be:
...
First, do we need a **causal** or **predictive** model? In our question above we have a causal model as we multiple outcomes with `att_end` based on our predictor: `treatment`.

Second, what is the outcome? `att_end` is the **outcome**. Note that the outcome is not necessarily the same concept mentioned in the questions.
...
As we did with the age ~ party we can take a different approach and instead of parameterizing this model without an intercept, we could have used one. 

To fit this model, we use the exact same code as before, except without the `-1` in the `formula` argument.

```{r}
stan_glm(att_end ~ treatment, 
         data = trains, 
         seed = 45,
         refresh = 0)
```

The intercept, 10.0, is the same as the `treatmentTreated` estimate in the first model. The `treatmentControl` estimate, which was previously 8.5, is now -1.5, meaning it is the difference (allowing for rounding) between the average age of the two treatments.
...
trains |> 
  ggplot(aes(x = income, fill = treatment)) +
    ...
    labs(title = "Expected Income Based On Treatment",
         ...
         y = "Probability",
         fill = NULL) +
    scale_x_continuous(labels = scales::dollar_format(scale = 1/10000)) +
    scale_y_continuous(labels = scales::percent_format()) + 
    theme_classic()
```

We can never know the true average income is of all treated in the population. But we can calculate a probability distribution for each parameter. As we can see with the plot above, the income stays relatively constant based on either the treatment. 
